...
const VALID_TYPES = new Set<PublicDocumentType>(['orcamento', 'ordem_servico', 'recibo'])
...
function normalizeDocumentType(value?: unknown): PublicDocumentType | null {
  ...
  if (['recibo', 'recebimento', 'receipt'].includes(normalized)) {
    return 'recibo'
  }
...
export async function POST(request: NextRequest) {
  ...
  const documentId = String(body.document_id || body.documentId || payload?.document_id || legacyPayload?.documentId || '')
  ...
  if (selectError) {
    console.error('Erro ao consultar public_documents para atualizacao:', selectError)
  }
