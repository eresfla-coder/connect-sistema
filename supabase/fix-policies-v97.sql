-- Connect Sistema V97 — CORREÇÃO: forçar recriação das policies

-- ============================================
-- LIMPAR TUDO E RECRIAR (cuidado: só em desenvolvimento!)
-- ============================================

-- Desativar RLS temporariamente para limpar
do $$
declare
  tabela text;
begin
  for tabela in 
    select tablename from pg_tables where schemaname = 'public' and tablename in ('clientes', 'produtos', 'orcamentos', 'ordens_servico', 'vendas', 'recibos', 'contratos', 'configuracoes_empresa')
  loop
    execute format('alter table if exists public.%I disable row level security;', tabela);
  end loop;
end;
$$;
...
-- ============================================
-- RECRIAR POLICIES PARA TODAS AS TABELAS
-- ============================================
